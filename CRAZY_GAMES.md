...
- Car 01 is the tutorial car: fewer people in the aisle, a longer clock, and holding forward follows the exit so a first try can clear it. Cars 02–05 add people and shorten the clock one step at a time (clocks 32, 32, 30, 30, 28 seconds). Door steer and the crowd shove fade by about a tenth each car, and the steer keeps the aisle line until the doorway is lined up, including the narrow door on car 04. Endless cars keep the original curve. A clear saves the highest car finished (`get-off-the-train.cg.best-clear`). The result card shows that ladder, the coins just earned, and the three upgrades.
