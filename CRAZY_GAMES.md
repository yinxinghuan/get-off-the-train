...
- Car 01 is the tutorial car: holding forward follows the exit, and a clean keyboard run leaves about 40% of the 10 second clock. Cars 02–05 add people and shorten the clock (10, 10, 10, 9, 9 seconds), so each later car leaves less time. Car 05 leaves about 10–20%. Door steer fades by about a tenth each car. Car 06 onward keeps stepping the same way, one second and a few riders at a time (car 06 is 8 seconds). A clear saves the highest car finished (`get-off-the-train.cg.best-clear`). The result card shows that ladder, the coins just earned, and the three upgrades.
