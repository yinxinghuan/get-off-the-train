...
- Car 01 is the tutorial car: fewer people in the aisle, a longer clock, and holding forward follows the exit so a first try can clear it. Cars 02–05 add people and shorten the clock one step at a time. The door steer and the crowd shove fade with each car instead of cutting off after car 02. Endless cars keep the original curve. A clear saves the highest car finished (`get-off-the-train.cg.best-clear`). The result card shows that ladder, the coins just earned, and the three upgrades.
