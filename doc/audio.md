...
- **Playback.** The loop and the stings are decoded with `decodeAudioData` and played through a music bus at gain 0.3, under the default master 0.8. The loop file is about −12 LUFS, so that bus lands it near −24 LUFS. Stings (−16 and −20 LUFS) get a 2× makeup gain on the way into the same bus so a clear still reads louder than the bed. The loop uses `AudioBufferSourceNode.loop`. A sting stops the loop; the next run starts the loop again. Effects stay on a separate bus (gain 0.7) as filtered oscillators and noise.
