...
`App.tsx` 维护 `playing / paused / level-clear / game-over`，并用 `runStarted` 区分“真实场景已显示但等待首次输入”和计时中的游戏。首次触摸或方向键输入才解锁声音、记录赛前最高分并启动倒计时；虚拟拇指只是 DOM 演示，不写入物理、分数或平台事件。前 5 关读取手工配置，之后由 `getLevelConfig()` 按封顶公式持续生成，挑战只有失败终点。每关用 `level` 作为 `TrainScene` key，确保所有物理实体和计时器独立重建。高频位置、速度、倒地计时和晃动阶段都留在 `useRef` 中，由 `useFrame` 更新；HUD 数据以 80 ms 间隔采样，游戏中把向上取整的整数秒渲染为常驻大倒计时，并在其下以一位小数渲染实时离门距离；关卡名依靠以 `level` 为 key 的 2.4 秒 CSS 进场动画短暂出现，摔倒数只用于内部逻辑和结算。
...
`buildTrain()` 根据每段长椅长度以 0.72–0.96 world-unit 间距生成 `seatSlots`，保存座位中心与面向通道的 yaw。关卡初始化时确定性随机留出 1–2 个空槽，其余从 `PROFESSION_KINDS` 选择职业并用 `makeSeatedProfessionPassenger()` 填满；函数把髋部对齐到 `y=0.75` 座面，将大腿前伸、小腿下垂，再按 30% 阅读、45% 看手机、25% 休息分配道具与姿态。坐席保存在独立 `seated` 数组，不加入圆形碰撞或主动 AI；主循环只更新 0.012 world-unit 呼吸和约 1° 的头肩侧摆，车身滚转由共同父节点自然传递。
