...
- `src/game/models.ts`：角色色盘与材质、几何图元、人物比例、地面锚点 / 髋部 / 上身 / 头颈 rig、上臂—前臂—手掌与大腿—小腿分段骨架、站姿与坐姿乘客工厂、乘客外形 roster。
...
`buildTrain()` 根据每段长椅长度以 0.72–0.96 world-unit 间距生成 `seatSlots`，保存座位中心与面向通道的 yaw。关卡初始化时确定性随机留出 1–2 个空槽，其余用 `makeSeatedPassenger()` 填满；函数把髋部对齐到 `y=0.75` 座面，将大腿前伸、小腿下垂、前臂落到膝上，并为约三分之二的坐席增加手机。坐席保存在独立 `seated` 数组，不加入圆形碰撞或主动 AI；主循环只更新 0.012 world-unit 呼吸和约 1° 的头肩侧摆，车身滚转由共同父节点自然传递。

配置乘客数乘以 0.68 得到通道站立人数。初始化时先分出 `clamp(round(n×12%),1,3)` 名 `exiting` 乘客，其余标记为 `stationary`：静止者在座椅前或立柱附近采样并面向通道，不创建移动目标；准备下车者从中央通道出发，目标 `x` 只递增，按 0.26–0.40 world-unit/s 逐段靠近侧门，每段抵达后停 1.8–3.8 秒，最终在门前变为静止者。玩家推挤、车辆冲量和摔倒仍可让任何站立者被动物理位移，但静止者速度衰减后不会自动返回、转身或随机换位。碰撞法向速度超过 0.55 时，根据关卡概率、门区压力、当前晃动余量和撞击速度决定是否 `knockDown(..., 'forward')`。摔倒状态记录 `fallStarted / fallDuration / fallKind`；世界空间的 `group` 只负责位置、碰撞、光环和灯光，永远不旋转，内部 `pose` 以脚底为枢轴，`upperBody` 以髋部为枢轴，`head` 以颈部为枢轴，上臂、前臂、手掌、大腿与小腿为嵌套关节。原始归一化时间 `rawP` 被分段重映射：前 46% 物理时间推进到姿态 0.58，接着 9% 推进到 0.68，最后 45% 完成剩余恢复；`key()` 在 11 个姿态节点间使用 Catmull–Rom 三次插值，保持跨节点的一阶速度连续。近侧手臂从姿态 0.06 开始伸出，远侧手臂延迟到 0.10，并分别使用独立的上臂与前臂曲线；前扑与侧摔也使用独立曲线，根姿态只保留约 20–22° 倾角，主要落地幅度由上身与四肢关节完成。物理滑动与倒计时继续，主动输入暂停；玩家前扑持续 1.18–1.34 秒、横摔 1.10 秒，乘客前扑持续 1.00–1.12 秒、横摔 0.94 秒；起身后玩家保护 1.0 秒、乘客保护 0.45 秒。角色视觉缩放为主角 0.58、普通/宽体 0.54、小体/幽灵 0.50、坐席 0.46–0.50，碰撞半径同步为 0.22 / 0.22 / 0.27 / 0.17；主角光环使用反向局部缩放维持 0.68 world-unit 识别尺寸。

站立人物的 `phase` 驱动错开的待机呼吸，主角振幅 0.052、站立乘客 0.032；默认静止者不累计步态。只有玩家、少量 `exiting` 乘客或碰撞造成的实际位移会累计 `gaitPhase`：主角每 1.05 world-unit、乘客每 0.58 world-unit 完成一轮左右脚。腿从髋部枢轴交替摆动和抬起，骨盆轻微扭转，上身反向补偿，手臂与对侧腿反摆；停止后平滑收腿。坐席乘客使用独立的低幅胸口呼吸，不套用站立弹跳。摔倒分支将步态与呼吸清零，减弱动态模式把振幅降至 35%。
