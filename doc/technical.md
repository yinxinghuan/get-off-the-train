...
通道站立人数前 5 关为 9 / 12 / 14 / 16 / 18，第 6 关起封顶 20；其中 `clamp(round(n×16%),2,4)` 名为 `wandering`，其余保持 `stationary`。前 5 关下车人数为 2 / 3 / 4 / 5 / 6；无限普通站按 `flowTier=floor((level-5)/5)` 从 5–7 人逐段增加并封顶 12 人，故事专列从 6–8 人逐段增加并封顶 12 人，持续上客同时安排 5–8 人逆向下车。上车仍从普通站 2–4 人、故事站 3–4 人逐步增加到 7 人，持续上客从 10 人增加到 14 人。`boarding` 首人于第 3.2–4.6 秒从绿门外出现，普通间隔由 2.4–2.85 秒逐步缩短到 1.6–2.05 秒，持续上客间隔由 1.05–1.35 秒缩短到 0.82–1.12 秒。坐席和站立下车者切换为 `exiting`，使用多条门内目标线；彼此碰撞间距压缩为正常值的 76%，使人群形成可流动的出门队列而不会永久堵死。人形静止者按确定性比例得到吊环、阅读、通话、手机或自然站姿，动物/怪物只保留自然姿态。碰撞法向速度超过 0.55 时，根据关卡、门区压力、晃动和撞击速度决定是否前扑。倒地动作使用 `pose / upperBody / head / arm / forearm / leg / shin` 分层 rig 与 Catmull–Rom 姿态曲线；所有角色外层视觉缩放为 0.58，体型差异来自内部比例；玩家与普通乘客碰撞半径 0.22。主角只保留红色通勤识别件与随身柔边 SpotLight，不创建 Torus 脚底环。

`models.ts` 用完整的 `Record<HeroId, MotionStyle>` 给 52 名共享库角色建立必填动作档案，TypeScript 会在新增角色却未配置动作时直接报错。GLB 加载后先克隆 `rig_legL / rig_legR / rig_armL / rig_armR` 的原始 Euler 角度到 `rig.rest`，逐帧循环只在这些基准角上叠加动作，避免覆盖僵尸前伸手臂、狼人前倾爪臂等库内造型。18 类动作基型覆盖普通、正式、老人、儿童、运动、重装、机甲、6 种怪物、四足、厚重四足、跳跃和摇摆，每个角色再按 catalog 序号获得稳定的 ±4.8% 节奏与 ±4.2% 重量差异，因此同类职业也不会完全同步。

站立人物的 `phase` 驱动错开的待机呼吸，主角振幅 0.052、站立乘客 0.032，再乘角色档案的 `bounce`。默认静止者不累计步态。只有玩家、少量 `wandering` 乘客或碰撞造成的实际位移会累计 `gaitPhase`：主角基础每 1.05 world-unit、乘客基础每 0.58 world-unit 完成一轮左右脚，再乘 `strideScale=0.68–1.38`。人形腿从髋部枢轴交替摆动和抬起，身体以角色自己的 `lean / sway / asymmetry` 组合前倾、摇摆和拖步；幽灵使用额外 0.065 world-unit 悬浮与低频漂移，动物在整个 `pose` 上分别叠加四足疾走、厚重步、跳跃或左右摇摆。静止者在同一段关节更新中叠加吊环、阅读、通话与看手机目标角度，并最终回到自身 `rig.rest` 而不是统一零姿态。坐席乘客使用独立的低幅胸口呼吸，不套用站立弹跳。摔倒分支暂时接管关节，起身结束后恢复原始姿态；减弱动态模式把动作振幅降至 35%。
