...
- `src/game/models.ts`：角色色盘与材质、几何图元、人物比例、外描边标记与乘客外形 roster。
...
关卡配置新增 `wander / tumbleChance / swayTumbleChance`，使难度由移动干扰和翻滚风险而非人数主导。非玩家每帧围绕 `homeX/homeZ` 选择带独立相位的移动目标，侧门前 3.5 world-unit 逐步扩大振幅和频率。碰撞法向速度超过 0.55 时，根据关卡概率、门区压力、当前晃动余量和撞击速度决定是否 `knockDown(..., 'tumble')`。翻滚状态记录 `fallStarted / fallDuration / fallKind`，用进度驱动 360° `rotation.x`、0.48 高度离地弧线和 0.18 侧摆；物理移动和倒计时继续，主动输入暂停。
