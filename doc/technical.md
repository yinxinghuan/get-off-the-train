...
3D Canvas 全屏响应式，55° 透视相机位置为 `(player.x-5.2, 4.8, player.z*0.35)`，视点为 `(player.x+4.8, 1.0, player.z*0.35)`；位置与视点共享相同 `z`，因此水平观察方向严格为世界 `+X`，不存在侧向透视。出口在 `x=8.27,z=0` 的端墙中央。HUD 和普通 DOM 按 320×568–430×932 内部重排，不依赖整页 transform。全屏输入面在每次 `pointerdown` 记录落指原点并显示 108 px 动态摇杆；屏幕向上固定映射 `+X`，左右固定映射 `±Z`。WASD / 方向键共用同一输入引用。

### 碰撞、晃动与更新

人物采用二维圆形刚体近似，保留 3D 视觉旋转。每帧限制 `dt<=33ms`，先处理主动速度和阻尼，再处理边界/立柱/行李静态碰撞，最后执行人物两两穿透分离和 35% 法向动量交换。玩家质量为 1.28，保证能推开普通乘客但仍会受宽体乘客和晃动影响。只有玩家在门宽内时，`x` 上限才从车厢边界放宽到门外 `9.05`；越过 `8.63` 即结算，避免旧版侧门边界把玩家夹在门口无法结束。周期晃动在预警窗口驱动吊环摆动，接触帧向所有人物施加横向冲量；失衡值超过个人稳定阈值时进入倒地、滑动、起身和短保护阶段。玩家松手 180 ms 后进入站稳状态，失衡降低 32%。

### 场景照明与主角识别

Canvas 使用 ACES Filmic 色调映射和 0.82 曝光，基础半球光强度 0.48、暖主光 1.15、冷轮廓填光 0.42。`buildTrain()` 在车体节点内生成 5 组暖白 PointLight 和可见灯管，因此光源会与车厢一起滚转；每帧以 0.72 Hz 缓慢调整灯强，其中一盏松动灯增加 2.15 Hz 起伏，冲击峰值按灯位短暂降至 38%。玩家模型自带 2.35 强度、4.2 world-unit 半径的暖色 PointLight，并叠加一个关闭深度写入的加法混合光环；局部光会照亮地板和身边乘客，DOM 的“你 / YOU”标签作为远距离和遮挡情况下的第二识别通道。
