...
`buildTrain()` 根据每段长椅长度以 0.72–0.96 world-unit 间距生成 `seatSlots`，保存座位中心与面向通道的 yaw。关卡初始化时确定性随机留出 1–2 个空槽，其余从 `PROFESSION_KINDS` 选择职业并用 `makeSeatedProfessionPassenger()` 填满；函数把髋部对齐到 `y=0.75` 座面，大腿局部 `rotation.x=-π/2` 水平朝通道，小腿用 `+π/2` 抵消父级旋转并沿局部 y 拉长 1.92 倍，使鞋落到地面；鞋组向前移 0.16 并把深度放大 1.35 倍，确保鞋尖伸出坐垫。坐席不加入可移动 `Body` 或主动 AI，但各自在面朝方向 0.52 world-unit 处向 `seated` 数组注册半径 0.18 的静态膝脚碰撞；主角重叠时沿法线推回，撞击速度超过 0.75 时播放碰撞声，并以 12% 概率触发 1.18 秒前扑。主循环另更新 0.012 world-unit 呼吸和约 1° 的头肩侧摆，车身滚转由共同父节点自然传递。
